...
class DataProcessor_offline():
    ...
    def run_proecss(self,raw_data,RAI_mode,Sure_staic_RM,chirp):
        ...
        while True:
            ...
            # range_resolution, bandwidth = mm.dsp.range_resolution(64,2000,100)
            ...
            radar_cube = mm.dsp.range_processing(raw_data, window_type_1d=Window.BLACKMAN)
            ...
            # fft2d_in,self.Y_last = self.delay_filter_src(fft2d_in,self.Y_last,a_weight=0.3)
            #
            self.frame_count += 1
            ...
            det_matrix, aoa_input = mm.dsp.doppler_processing(fft2d_in, num_tx_antennas=3,
                                                              ...
                                                              window_type_2d=Window.BLACKMAN,
                                                               accumulate=False,interleaved=False)
            ...
            if snr_mode == 1:
                ...
                SNRThresholds2 = np.array([[2, 30], [10, 20], [35, 16]])
                peakValThresholds2 = np.array([[0, 400], [1, 400], [500, 0]])

            # detObj2D = mm.dsp.range_based_pruning(detObj2D, SNRThresholds2, peakValThresholds2, 58, 53, # 64== numRangeBins
            # detObj2D = mm.dsp.range_based_pruning(detObj2D, SNRThresholds2, peakValThresholds2, 64, 58, # 64== numRangeBins
            #                                       range_resolution)
